...
const findPhoto = (row: Record<string, unknown> | null): PhotoResult | null => {
  if (!row) return null

  for (const field of PHOTO_FIELDS) {
    const value = row[field]
    if (looksLikeImage(value)) return { field, url: String(value), raw: String(value) }
  }
  ...
  for (const field of PHOTO_FIELDS) {
    const value = row[field]
    if (typeof value === 'string' && value.trim()) return { field, url: '', raw: value.trim() }
  }
      ...
      <div className="bg-white border-b border-a2o-warm sticky top-0 z-40">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-lg font-serif font-bold">客人相片查看</div>
              <p className="text-xs text-a2o-black/40">逐個客人按需要先載入相片，避免後台一次過載入所有圖片</p>
            </div>
          </div>
                    ...
                    <div className="mt-4 bg-a2o-beige rounded-xl p-3">
                      {photo?.url ? (
                        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                          ...
                          <div className="space-y-2 min-w-0">
                            ...
                            <div className="flex flex-wrap gap-2">
                              ...
                              <button onClick={() => copyValue(photo.raw)} className="px-3 py-2 border border-a2o-warm bg-white rounded-lg text-xs">複製連結 / 圖片資料</button>
                            </div>
                          </div>
                        </div>
                      ) : photo?.raw ? (
                        <div className="space-y-2">
                          ...
                          <p className="text-xs text-a2o-black/50">來源欄位：{photo.field}</p>
                          <button onClick={() => copyValue(photo.raw)} className="px-3 py-2 border border-a2o-warm bg-white rounded-lg text-xs">複製原始相片資料</button>
                        </div>
                      ) : (
